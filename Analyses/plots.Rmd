...
p4 <- effects %>% rename(`p-value` = p.value) %>% ggplot(aes(x = Effects, y = Term, shape = `p-value`,xmax = Ub, xmin = Lb))  +
  facet_grid(~ POS) +
  geom_vline(xintercept = 0, color = "grey", linetype = "dotted", size = 1) +
  ...
  theme_bw()+
  xlim(-1,1)+
  theme(strip.background = element_rect(colour="#FFFFFF", fill="#FFFFFF"),legend.position="bottom",text=element_text(size=12,  family="Times New Roman"), axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12), strip.text.x = element_text(size = 18,face = "bold"),
        plot.margin = margin(1, 0.5, 0.5, 0.5, "cm"))

ggsave("plots_exp4_model_aoa_diff.png",plot=p4, width = 9, height = 4, units="in", limitsize = FALSE)
```
