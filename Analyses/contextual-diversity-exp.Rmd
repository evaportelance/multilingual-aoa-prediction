...
data <- data %>% unique() |> group_by(language,uni_lemma,definition) |> mutate(surprisal_1gm = - log(all_frequency)) |> mutate_at(vars(starts_with("surprisal"),lstm_surprisal,avg_perplexity), ~sum(.*all_frequency)/sum(all_frequency)) |> mutate(all_frequency=sum(all_frequency)) |> mutate(log_cd_score=log(cd_score)) |> select(language, uni_lemma, category, definition, lexical_class, lstm_surprisal, surprisal_2gm, surprisal_3gm, surprisal_4gm, surprisal_1gm, cd_score, log_cd_score) |> unique() |> ungroup() |> group_by(language) |> mutate_at(vars(c("lstm_surprisal", "surprisal_1gm", "surprisal_2gm", "surprisal_3gm", "surprisal_4gm", "cd_score", "log_cd_score")), ~as.numeric(base::scale(.)))
